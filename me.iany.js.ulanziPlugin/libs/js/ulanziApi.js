...
class UlanziStreamDeck {
    ...
    connect(uuid) {
        ...
        Utils.log(
            `[ULANZIDECK] ${this.isMain ? "MAIN" : "CLIENT"} WEBSOCKET CONNECT:${this.uuid
            }`
        );
        ...
        this.websocket.onopen = () => {
            Utils.log(
                `[ULANZIDECK] ${this.isMain ? "MAIN" : "CLIENT"} WEBSOCKET OPEN:${this.uuid
                }`
            );
            const json = {
                code: 0,
        ...
        this.websocket.onerror = (evt) => {
            const error = `[ULANZIDECK] ${this.isMain ? "MAIN" : "CLIENT"
        ...
        this.websocket.onclose = (evt) => {
            Utils.warn(
                `[ULANZIDECK] ${this.isMain ? "MAIN" : "CLIENT"} WEBSOCKET CLOSED:${SocketErrors["DEFAULT"]
                }`
            );
        ...
        this.websocket.onmessage = (evt) => {
            ...
            Utils.log(
                `[ULANZIDECK] ${this.isMain ? "MAIN" : "CLIENT"
